...
void GameCore::addTime(Real deltaTime)
{
...
void GameCore::setupCamera(Camera *cam)
{
    ...
    mCameraNode->setAutoTracking(true, mBodyNode);
...
void GameCore::injectKeyDown(const OIS::KeyEvent& evt)
{
    ...
    if (evt.key == OIS::KC_SPACE && (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP))
    {
    	// jump if on ground
...
void GameCore::injectKeyUp(const OIS::KeyEvent &evt)
{
    switch (evt.key)
    {
        case OIS::KC_W:
...
void GameCore::injectMouseMove(const OIS::MouseEvent& evt)
{
    // update camera goal based on mouse movement
    updateCameraGoal(-0.05f * evt.state.X.rel, -0.05f * evt.state.Y.rel, -0.0005f * evt.state.Z.rel);
}

void GameCore::injectMouseDown(const OIS::MouseEvent& evt, OIS::MouseButtonID id)
{
...
void GameCore::setupBody(SceneManager *sceneMgr)
{
    // create main model
...
void GameCore::updateBody(Real deltaTime)
{
...
void GameCore::updateAnimations(Real deltaTime)
{
    ...
    if (mTopAnimID == ANIM_DRAW_SWORDS)
    {
        ...
        if (mTimer >= mAnims[mTopAnimID]->getLength() / 2 && mTimer - deltaTime < mAnims[mTopAnimID]->getLength() / 2)
        {
            ...
            // change the hand state to grab or let got
...
void GameCore::fadeAnimations(Real deltaTime)
{
    for (int i = 0; i < NUM_ANIMS; i++)
    {
        if (mFadingIn[i])
        {
            ...
            mAnims[i]->setWeight(std::min<Real>(newWeight, 1));
            if (newWeight >= 1) 
                mFadingIn[i] = false;
        }
...
void GameCore::updateCamera(Real deltaTime)
{
...
void GameCore::updateCameraGoal(Real deltaYaw, Real deltaPitch, Real deltaZoom)
{
...
void GameCore::setBaseAnimation(AnimID id, bool reset)
{
...
void GameCore::setTopAnimation(AnimID id, bool reset)
{
